...
//LG1 consider repacing with enter data  
#ifndef USE_UVM___
   m_regNumList = new Index_t[numElem()] ;  // material indexset
#else
  cudaMallocManaged(&m_regNumList, numElem()*sizeof(Index_t));
#endif
   ...
   if (USE_DEVICE){
     fx_elem_WORK = (Real_t*)  omp_target_alloc(8*numElem()*sizeof(Real_t),omp_get_default_device());
     fy_elem_WORK = (Real_t*)  omp_target_alloc(8*numElem()*sizeof(Real_t),omp_get_default_device());
     fz_elem_WORK = (Real_t*)  omp_target_alloc(8*numElem()*sizeof(Real_t),omp_get_default_device());


     x8n_WORK = (Real_t*)  omp_target_alloc(8*numElem()*sizeof(Real_t),omp_get_default_device());
     y8n_WORK = (Real_t*)  omp_target_alloc(8*numElem()*sizeof(Real_t),omp_get_default_device());
     z8n_WORK = (Real_t*)  omp_target_alloc(8*numElem()*sizeof(Real_t),omp_get_default_device());
   }

//   cudaMallocManaged(&fx_elem_WORK,8*numElem());
//   cudaMallocManaged(&fy_elem_WORK,8*numElem());
//   cudaMallocManaged(&fz_elem_WORK,8*numElem());
...
Domain::SetupThreadSupportStructures()
{
  ...
  if ( (numthreads > 1) || (USE_DEVICE) ) {
    ...
    Index_t* ptr;
    cudaMallocManaged(&ptr,(numNode()+1)*sizeof(Index_t));
    m_nodeElemStart = new(ptr) Index_t[numNode()+1] ;
    ...
    cudaMallocManaged(&ptr,m_nodeElemStart[numNode()]*sizeof(Index_t));    
    m_nodeElemCornerList = new(ptr) Index_t[m_nodeElemStart[numNode()]];
...
#ifdef USE_DEVICE
  Real_t *ptr;
  cudaMallocHost(&ptr, comBufSize*sizeof(Real_t));
  this->commDataSend = new(ptr) Real_t[comBufSize] ; 
  cudaMallocHost(&ptr, comBufSize*sizeof(Real_t));
  this->commDataRecv = new(ptr) Real_t[comBufSize] ;

  cudaMalloc(&ptr, comBufSize*sizeof(Real_t));
  this->commDataSend_device = ptr;
  cudaMalloc(&ptr, comBufSize*sizeof(Real_t));
  this->commDataRecv_device = ptr;
...
#else
    cudaMallocManaged(&m_regElemlist, numReg()*sizeof(Index_t*));
    m_regElemlist = new(m_regElemlist)  Index_t*[numReg()];
#endif
   Index_t nextIndex = 0;
...
#else
    Index_t *ptr = m_regElemlist[i];
    cudaMallocManaged(&ptr, regElemSize(i)*sizeof(Index_t));
    m_regElemlist[i] = new(ptr) Index_t[regElemSize(i)]; 
#endif
      regElemSize(i) = 0;
   }
