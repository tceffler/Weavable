...
!_______________________________________________________________________
    ...
    INTEGER(i_knd) :: ierr, ranks_per_node, local_rank
!_______________________________________________________________________
!
!   Set thread support levels if you have a thread multiple MPI.  These and the
!   MPI_INIT_THREAD are commented out since they are not supported everywhere.
!   Comment them back in if you have a compiler with this support and want to
!   use them.
!_______________________________________________________________________
...
!_______________________________________________________________________
    ...
    call bindthreads(ranks_per_node, local_rank)
    CALL WTIME ( t1 )

#ifdef USE_COMM2D
    call get_2d_communicator(comm_snap)
#else
...
!_______________________________________________________________________
    ...
    dims(2) = npey
    periodic = .FALSE.
    reorder = .FALSE.   !YKT : set to false and use a re-ordered comm_snap
...
!    write(6,*) 'max_threads=',max_threads
