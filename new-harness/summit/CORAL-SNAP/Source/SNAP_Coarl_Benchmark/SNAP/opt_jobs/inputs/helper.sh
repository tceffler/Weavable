...
#--------------------------------------------------------------------------------
 ...
 cpus_per_node=160
 declare -a list0=(`seq 0 4 76`)
 declare -a list1=(`seq 88 4 164`)
...
# set some variables specifically for optimized SNAP
...
if [ $socket = 0 ]; then
  let ndx=$local_rank*$cpus_per_rank/4
  let start_cpu=${list0[$ndx]}
  let stop_cpu=$start_cpu+$cpus_per_rank-4
else
  let rank_in_socket=$local_rank-$ranks_per_socket
  let ndx=$rank_in_socket*$cpus_per_rank/4
...
#-------------------------------------------------
 printf -v command "./numactl --membind=%d"  $numa_node
