...
!_______________________________________________________________________
!
!  Init GPU and allocate GPU data
!_______________________________________________________________________
    IF ( use_gpu ) THEN
  !!$OMP PARALLEL
      CALL setgpu ( iproc )
  !!$OMP END PARALLEL
      CALL setup_cuda ( timedep, iproc, nang, ichunk, nx, ny, nz, ng, noct, &
           cmom, nmom, src_opt, nmat, ycomm, zcomm, npey, npez, yproc, zproc )
    END IF
!_______________________________________________________________________
!
...
!_______________________________________________________________________
!
!   Transfer data that won't change during time stepping to GPU
!_______________________________________________________________________

    IF ( use_gpu ) THEN
      CALL h2d_eta ( eta, nang )
      CALL h2d_xi ( xi, nang )
      CALL h2d_mu ( mu, nang )
      CALL h2d_w ( w, nang )
      CALL h2d_mat ( mat, nx, ny, nz )
      CALL h2d_qi ( qi, nx, ny, nz, ng )
      CALL h2d_sigt ( sigt, nmat, ng )
      CALL h2d_siga ( siga, nmat, ng )
      CALL h2d_slgg ( slgg, nmat, nmom, ng )
      CALL h2d_lma ( lma, nmom )
      CALL h2d_ec ( ec, nang, cmom, noct ) 
      IF ( src_opt == 3 ) CALL h2d_qim ( qim,nang,nx,ny,nz,noct,ng )
    END IF
