...
!    IF ( ndimen>1 .AND. ny /= 16*npey ) THEN
!      ierr = ierr + 1
!      error = '***ERROR: INPUT_CHECK: This version requires NY = 16*NPEY'
!      CALL print_error ( ounit, error )
!    END IF
...
!    IF ( ndimen>2 .AND. nz /= 16*npez ) THEN
!      ierr = ierr + 1
!      error = '***ERROR: INPUT_CHECK: This version requires NZ = 16*NPEZ'
!      CALL print_error ( ounit, error )
!    END IF
...
!_______________________________________________________________________
    ...
    IF ( MOD(nang,16) /= 0 ) THEN
      ierr = ierr + 1
      error = '***ERROR: This version requires NANG to be multiple of 16'
      CALL print_error ( ounit, error )
    END IF
!_______________________________________________________________________
!
