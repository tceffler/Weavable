...
MODULE octsweep_module
  ...
  USE sn_module, ONLY: ec, nang, wmu, weta, wxi, noct, cmom, w, mu

  USE data_module, ONLY: vdelt, src_opt, qim, ng
...
!_______________________________________________________________________
    ...
    ELSE
      CALL dim3_sweep ( ich, id, d1, d2, d3, d4, jd, kd, jlo, klo,     &
        ...
        flkz(:,:,:,g), t_xs(:,:,:,g) )
    END IF
!!$    call dim3_sweep_cuda(iop,nc,g,ichunk,ndimen,noct,&
!!$         nang,nx,ny,nz,jst,kst,cmom,src_opt,ng,&
!!$         jlo,klo,jhi,khi,jd,kd,&
!!$         lasty,lastz,firsty,firstz,timedep,&
!!$         vdelt(g),w,&
!!$         hi,hj,hk,mu,&
!!$         psii(:,:,:,g),psij(:,:,:,g),psik(:,:,:,g),&
!!$         jb_in(:,:,:,g),kb_in(:,:,:,g),jb_out(:,:,:,g),kb_out(:,:,:,g),&
!!$         ptr_in(:,:,:,:,i1,i2),ptr_out(:,:,:,:,i1,i2),&
!!$         qtot(:,:,:,:,g),qim,ec,dinv(:,:,:,:,g),&
!!$         flux(:,:,:,g), fluxm(:,:,:,:,g))
