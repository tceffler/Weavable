...
!-----------------------------------------------------------------------
  ...
  USE geom_module, ONLY: geom_alloc, geom_dealloc, dinv, param_calc,   &
    nx, ny, nz, ny_gl, nz_gl, diag_setup, dx, dy, dz, nc, hi, ndimen

  USE sn_module, ONLY: nang, noct, mu, eta, xi, cmom, nmom

  USE data_module, ONLY: ng, v, vdelt, mat, sigt, siga, slgg, src_opt, &
    qim, nmat

  USE control_module, ONLY: nsteps, timedep, dt, oitm, otrdone,        &
    control_alloc, control_dealloc, dfmxo, it_det, use_gpu
...
!_______________________________________________________________________
  ...
  time_loop: DO cy = 1, nsteps
    if (iproc == root) print *, 'time step = ', cy
    CALL wtime ( t3 )
    ...
    IF ( timedep == 1 ) THEN
      ...
      vdelt = two / ( dt * v )
      IF ( use_gpu ) CALL h2d_vdelt ( vdelt, ng )
...
!_______________________________________________________________________
    ...
    IF ( src_opt == 3 ) THEN
      IF ( use_gpu .EQV. .FALSE. ) THEN
        ...
        END IF
      ELSE 
        CALL scale_qim_cuda ( qim, nang, nx, ny, nz, noct, ng, cy, time )
      END IF
    END IF
!_______________________________________________________________________
!
...
!_______________________________________________________________________
    ...
    outer_loop: DO otno = 1, oitm
      ...
      if (iproc == root) print *, 'outer = ', otno
...
!_______________________________________________________________________
  ...
  !$OMP END PARALLEL DO
      ELSE
        CALL expxs_sigt_cuda ( nx, ny, nz, nmat, ng )
        CALL expxs_siga_cuda ( nx, ny, nz, nmat, ng )
        CALL expxs_slgg_cuda ( nmat, nmom, ng, nx, ny, nz )
        CALL param_calc_cuda ( ichunk, nx, ny, nz, nc, dx, dy, dz, &
             nang, ng, ndimen, hi )
      END IF
!_______________________________________________________________________
!
...
!_______________________________________________________________________
      ...
      !CALL dump_flux(nx,ny,nz,cy,otno)
!      CALL dump_flux(nx,ny,nz,otno,otno)
!      CALL dump_fluxm(nx,ny,nz,cmom,otno,otno)
