...
!_______________________________________________________________________
    ...
    IF ( use_gpu .EQV. .FALSE. ) THEN
      CALL otr_src
    ELSE
      CALL otr_src_cuda ( nang, nx, ny, nz, ng, cmom, nmom, nmat, q2grp )
    END IF
...
!_______________________________________________________________________
!
!   Copy updated flux from GPU to CPU as it's used in otr_conv
!_______________________________________________________________________

    IF ( use_gpu ) CALL d2h_flux(flux, fluxm, cmom, nx, ny, nz, ng)
