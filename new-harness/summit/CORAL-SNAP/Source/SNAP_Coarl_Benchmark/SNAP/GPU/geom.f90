...
MODULE geom_module
  ...
  USE control_module, ONLY: use_gpu
...
! local variables for CUDA 
    INTEGER(i_knd), ALLOCATABLE, DIMENSION(:) :: diag_len, diag_ic, diag_j, diag_k, diag_count
    INTEGER(i_knd) :: tot_size, d, n
!_______________________________________________________________________
!
...
! GPU version always need nested
    IF ( do_nested .or. use_gpu ) THEN
!    IF ( do_nested ) THEN
...
!_______________________________________________________________________
    ...
    IF ( use_gpu ) THEN
      ALLOCATE( diag_len(ndiag), diag_count(ndiag) )
      tot_size = 0
      diag_count(1) = 0
      DO d = 1, ndiag
        tot_size = tot_size + diag(d)%len
        diag_len(d) = diag(d)%len
        IF (d >= 2) diag_count(d) = diag_count(d-1) + diag(d-1)%len
      END DO
      ALLOCATE(diag_ic(tot_size), diag_j(tot_size), diag_k(tot_size))
      DO d = 1, ndiag
        DO n = 1, diag(d)%len
          diag_ic(diag_count(d) + n) = diag(d)%cell_id(n)%ic
          diag_j(diag_count(d) + n) = diag(d)%cell_id(n)%j
          diag_k(diag_count(d) + n) = diag(d)%cell_id(n)%k
        END DO
      END DO

      CALL h2d_diag ( diag_len, diag_ic, diag_j, diag_k, diag_count,&
           ndiag, tot_size )
...
!       end do
!       stop
      DEALLOCATE(diag_len, diag_count, diag_ic, diag_j, diag_k)
    ENDIF
!_______________________________________________________________________
!_______________________________________________________________________
