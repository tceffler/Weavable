...
MODULE sweep_module
  ...
  USE geom_module, ONLY: ndimen, nx, ny, nz, nc, hi

  USE sn_module, ONLY: nang, noct, cmom

  USE data_module, ONLY: ng, src_opt

  USE control_module, ONLY: inrdone, timedep, use_gpu, fixup, tolr
...
!_______________________________________________________________________
      ...
      IF ( use_gpu ) THEN
        DO i = 1, num_grth
          ...
          dogrp(i) = g
        END DO
        CALL h2d_dogrp(dogrp, ng)
!!$        DO iop = 1, 2*nc
!!$          call dim3_sweep_cuda(dogrp,num_grth,iop,nc,ichunk,&
!!$               ndimen,noct,nang,nx,ny,nz,jst,kst,cmom,src_opt,ng,&
!!$               jlo,klo,jhi,khi,jd,kd,&
!!$               lasty,lastz,firsty,firstz,timedep,hi, &
!!$               mtag, yp_rcv, yp_snd, yproc, zp_rcv, zp_snd, zproc,&
!!$               ycomm, zcomm, iproc)
!!$        END DO
!          write (*,*) 'zproc=',zproc,' jd=',jd,' kd=',kd
          call dim3_sweep_cuda(dogrp,num_grth, nc, ndimen, jst,kst, src_opt, fixup, &
               jlo,klo,jhi,khi,jd,kd,&
               lasty,lastz,firsty,firstz,timedep,tolr,hi, &
               mtag, yp_rcv, yp_snd, yproc, zp_rcv, zp_snd, zproc,&
               iproc)
