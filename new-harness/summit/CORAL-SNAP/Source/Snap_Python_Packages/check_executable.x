...
# Modify the python path for local SNAP python harness packages.
sys.path.append('../../Source/Snap_Python_Packages') 

# Local imports
from snap_job_verifier import SnapGPUVerifier

def check_executable(path_to_results,
                     test_id_string):

    #-----------------------------------------------------
    # Create an instance of a sanp gpu verifier.         -
    #                                                    -
    #-----------------------------------------------------
    my_gpu_snap_verifier = SnapGPUVerifier(path_to_results,
                                           test_id_string)

    #-----------------------------------------------------
    # Validate that the tests meets the throughput       -
    # metric.                                            -
    #                                                    -
    #-----------------------------------------------------
    jstatus = my_gpu_snap_verifier.validate()

    #
    # Write the status of the results to job data file.
    #
    write_to_job_data(path_to_results,
                      jstatus)
    return
