...
//extern "C" void bindthreads(int *, int *);

#define R_VALUE_OFFSET 2

int main(int argc, char *argv[])
{
  int world_size, local_rank, world_rank, my_group;
  ...
  int ranks_per_node, rank_in_node;
  ...
  //bindthreads(&ranks_per_node, &rank_in_node);

  int numdevices, mydevice;
  cudaGetDeviceCount(&numdevices);
  mydevice = world_rank % numdevices;
  cudaSetDevice(mydevice);

  ev0[0]=ev0[1]=0.0; ev0[2]=1.0;
  // world_size has to be align + size_lsms*num_lsms
  align=1;
  num_lsms=1;
  size_lsms=-1;
  my_group=-1;
...
#define NUM_PAPI_EVENTS 4
  ...
  // IBM: add a barrier to ensure accurate timing
...
//      generator_needs_moment = true;
  ...
  else
  {
    ...
    int s=align;
    int comm_size=(world_size-align)/num_lsms;
    for(int i=0; i<num_lsms; i++)
    {
      if((world_rank>=s) && (world_rank<s+comm_size)) { my_group=i; color=i;}
      lsms_rank0[i]=s;
      s+=comm_size;
    }
...
//  YKT: replace bcast with simple send/recv loop
/*
    int done_flag = 1;
    int tag = 99;
    if (world_rank == 0) {
       for (int i=1; i<world_size; i++) {
          MPI_Recv(&done_flag, 1, MPI_INT, MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &status);
          fprintf(stderr, "YKT: num recv = %d\n", i);
       }
    }
    else {
      MPI_Send(&done_flag, 1, MPI_INT, 0, tag, MPI_COMM_WORLD);
      fprintf(stderr, "YKT: rank %d send done\n", world_rank);
    }
*/
