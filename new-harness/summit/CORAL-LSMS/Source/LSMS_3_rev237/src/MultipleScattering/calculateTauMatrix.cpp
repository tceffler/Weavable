...
#endif
{
  ...
  double timeBuildKKRMatrix;

  // build the kkr matrix
  m.resize(nrmat_ns,nrmat_ns);
  
//IBM: change if (lsms.global.iprint>=0) => if (lsms.global.iprint>0) 
//to limit output in large parallel jobs
  if (lsms.global.iprint>0) timeBuildKKRMatrix=MPI_Wtime();
