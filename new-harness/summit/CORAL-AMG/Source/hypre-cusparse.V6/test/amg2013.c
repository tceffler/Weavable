...
//mpi communicator
MPI_Comm HP_MPI_COMM;

#ifdef SWITCH_HOST_DEVICE
   int SWITCH_LEVEL = 6; //default
#endif
...
void set_dscr(int * arg)
{
   unsigned long val = *arg;
   asm volatile("mtspr %1,%0" : : "r" (val), "i" (SPRN_DSCR));
}

void set_dscr_env(void)
{
   char * ptr;
   ptr = getenv("POWER_DSCR");
   if (ptr != NULL) {
      unsigned long val = (unsigned long) atoi(ptr);
      asm volatile("mtspr %1,%0" : : "r" (val), "i" (SPRN_DSCR));
   }
}
...
// debug: timing per level
extern double timing[20];

int gpu_level = 0;
int mype, npe;

// debug: host only flag for GPU run
int host_only = 0;

/*
int getNumRowsnnz(int rows, int *I)
{
  int num_rowsnnz = 0;

  int i;
  for (i=0; i<rows; i++) {
    if ((I[i+1]-I[i]) != 0) num_rowsnnz++;
  } 
  return num_rowsnnz;
}
*/

int hierarchy_num_levels(void *amg_vdata)
{
  hypre_ParAMGData *amg_data = amg_vdata;
  return hypre_ParAMGDataNumLevels(amg_data);
}

int print_all_timing(int num_levels)
{
  double max_times[20];
  ...
  for (i=0; i<num_levels; i++) {
    MPI_Reduce(&timing[i], &max_times[i], 1, MPI_DOUBLE, MPI_MAX, 0, HP_MPI_COMM);
  }
  if (myid == 0) {
    for (i=0; i<num_levels; i++) {
      printf("LEVEL_TIME %d %lg\n",i,max_times[i]);
    }
  }
}

/*--------------------------------------------------------------------------
 * upload relevant PCG parts
 *--------------------------------------------------------------------------*/
void upload_PCG(void *pcg_vdata)
{
  hypre_PCGData *pcg_data = (hypre_PCGData *)pcg_vdata;

  hypre_ParVector *r = (hypre_ParVector *)pcg_data->r;
  hypre_ParVector *s = (hypre_ParVector *)pcg_data->s;
  hypre_ParVector *p = (hypre_ParVector *)pcg_data->p;

  hypre_VectorSyncDevice(r->local_vector);
  hypre_VectorSyncDevice(s->local_vector);
  hypre_VectorSyncDevice(p->local_vector);
}

/*--------------------------------------------------------------------------
 * upload entire hierarchy to device after setup phase
 *--------------------------------------------------------------------------*/
void upload_hierarchy(void *amg_vdata, int rlx)
{
  if (GPU_SOLVE_PHASE) {
    hypre_ParAMGData *amg_data = amg_vdata;
    ...
    int amg_logging = hypre_ParAMGDataLogging(amg_data);
    hypre_ParVector *residual = hypre_ParAMGDataResidual(amg_data);
    hypre_ParVector *Vtemp    = hypre_ParAMGDataVtemp(amg_data);
    hypre_ParVector *Ztemp    = hypre_ParAMGDataZtemp(amg_data);

    if (amg_logging > 1) {
      hypre_VectorSyncDevice(residual->local_vector);
    }
    hypre_VectorSyncDevice(Vtemp->local_vector);
    
    if (rlx == 16) {
      hypre_VectorSyncDevice(Ztemp->local_vector);
    }
    ...
    int level_is_set = 0;
    for (i=0; i<num_levels; i++) {
      int rows, row_nnz, cols, nnz, *I, *J, total_nnz=0, temp;
      double *data;
      // upload vectors
      hypre_VectorSyncDevice(U_array[i]->local_vector);
      hypre_VectorSyncDevice(F_array[i]->local_vector);

      // upload A (diag)
      hypre_CSRMatrix *diag = hypre_ParCSRMatrixDiag(A_array[i]);

      rows = hypre_CSRMatrixNumRows(diag);
      cols = hypre_CSRMatrixNumCols(diag);
      nnz  = hypre_CSRMatrixNumNonzeros(diag);
      data = hypre_CSRMatrixData(diag);
      I = hypre_CSRMatrixI(diag);
      J = hypre_CSRMatrixJ(diag);

      hypre_CSRMatrixSetRownnz(diag);
      row_nnz = hypre_CSRMatrixNumRownnz(diag);

//set cut-off for V-cycle for solving on GPU 

#ifdef SWITCH_HOST_DEVICE
       //set level for switching execution to/from DEVICE
       if ((nnz < 40000) && (level_is_set == 0) ){
           SWITCH_LEVEL = i;
           level_is_set=1;
           // printf("rank= %d: SWITCH_LEVEL = %d\n",myid,SWITCH_LEVEL);
       }
#endif


      mem_usage += (rows+1)*sizeof(int) + nnz*(sizeof(int) + sizeof(double));
      // vector mem_usage
      mem_usage += 5*rows*sizeof(double) + cols*sizeof(double);
      // printf("P(%d): memory usage: %f GB\n", myid, (double)mem_usage/pow(1024.0,3));
      gpu_level = i;
      device_create_matrix(diag, rows, cols, nnz, I, J, data, cols, rows, 0); 
      device_create_diagonal(diag);
      device_get_vector(rows, 0);
      device_get_vector(rows, 1);
      device_get_vector(rows, 2);
      if (l1_norms != NULL)
        device_set_l1_norms(diag, l1_norms[i]);
      // mem_usage += rows*sizeof(double);
#ifndef GPU_USE_CUSPARSE_HYB
      if (row_nnz < xpar * rows) {
        //printf("Setting hyper-sparse matrix (%d x %d)\n");
        device_set_hyper_sparse(diag, hypre_CSRMatrixNumRownnz(diag), hypre_CSRMatrixRownnz(diag));
        mem_usage += hypre_CSRMatrixNumRownnz(diag)*sizeof(int);
      }
#endif
      total_nnz = nnz;

      // upload A (off-diag)
      hypre_CSRMatrix *offd = hypre_ParCSRMatrixOffd(A_array[i]);

      rows = hypre_CSRMatrixNumRows(offd);
      cols = hypre_CSRMatrixNumCols(offd);
      nnz  = hypre_CSRMatrixNumNonzeros(offd);
      data = hypre_CSRMatrixData(offd);
      I = hypre_CSRMatrixI(offd);
      J = hypre_CSRMatrixJ(offd);

      hypre_CSRMatrixSetRownnz(offd);
      row_nnz = hypre_CSRMatrixNumRownnz(offd);

      device_create_matrix(offd, rows, cols, nnz, I, J, data, cols, rows, 0);
      // device_create_diagonal(rows, cols);
      mem_usage += (rows+1)*sizeof(int) + nnz*(sizeof(int) + sizeof(double));
      // if (rows) device_create_diagonal(rows, cols);
      if (row_nnz < xpar * rows) {
        //printf("Setting hyper-sparse matrix (%d x %d)\n");
        device_set_hyper_sparse(offd, hypre_CSRMatrixNumRownnz(offd), hypre_CSRMatrixRownnz(offd));
        mem_usage += hypre_CSRMatrixNumRownnz(offd)*sizeof(int);
      }

      // vector mem_usage
      mem_usage += (rows+cols)*sizeof(double);
      total_nnz += nnz;

      // get global max nnz
      int max_nnz;

      MPI_Reduce(&total_nnz, &max_nnz, 1, MPI_INT, MPI_MAX, 0, HP_MPI_COMM);
      int myid;
      MPI_Comm_rank(HP_MPI_COMM, &myid);
      if (myid == 0) {
        printf("MAX_NNZ %d %d\n",i,max_nnz);
      ...
      if (i != num_levels-1) {
        // upload P, R (diag)
        diag = hypre_ParCSRMatrixDiag(P_array[i]);
        if (rows) {
          rows = hypre_CSRMatrixNumRows(diag);
          cols = hypre_CSRMatrixNumCols(diag);
          nnz  = hypre_CSRMatrixNumNonzeros(diag);
          ...
          device_create_matrix(diag, rows, cols, nnz, I, J, data, cols, rows, 0);
          device_create_matrix(diag, rows, cols, nnz, I, J, data, rows, cols, 1);
          mem_usage += (rows+1)*sizeof(int) + nnz*(sizeof(int) + sizeof(double));
          mem_usage += (cols+1)*sizeof(int) + nnz*(sizeof(int) + sizeof(double));
          if (row_nnz < xpar * rows) {
            // printf("Setting hyper-sparse matrix (%d x %d)\n");
            device_set_hyper_sparse(diag, hypre_CSRMatrixNumRownnz(diag), hypre_CSRMatrixRownnz(diag));
            mem_usage += hypre_CSRMatrixNumRownnz(diag)*sizeof(int);
        ...
        // upload P, R (off-diag)
        offd = hypre_ParCSRMatrixOffd(P_array[i]);
        ...
        if (rows) {
          ...
          device_create_matrix(offd, rows, cols, nnz, I, J, data, cols, rows, 0);
          device_create_matrix(offd, rows, cols, nnz, I, J, data, rows, cols, 1);
          mem_usage += (rows+1)*sizeof(int) + nnz*(sizeof(int) + sizeof(double));
          mem_usage += (cols+1)*sizeof(int) + nnz*(sizeof(int) + sizeof(double));
          if (row_nnz < xpar * rows) {
            // printf("Setting hyper-sparse matrix (%d x %d)\n");
    ...
    // printf("P(%d)-L%d: free = %f, total = %f\n", myid, i, (double)free/pow(1024.0,3), (double)total/pow(1024.0,3));
      device_report_memory();
    } // end loop over levels
    // create cublas here to not include in the solve time
    device_createCublas();

    // sum up memory usage over all ranks
    // double total_mem_usage = 0.;
    // MPI_Reduce(&mem_usage, &total_mem_usage, 1, MPI_DOUBLE, MPI_SUM, 0, HP_MPI_COMM);
    // if (myid == 0) {
    //   printf("MEM_PER_FINE_ROW %.5e GB\n",total_mem_usage / 1e9 / hypre_ParCSRMatrixGlobalNumRows(A_array[0]));
    // }
    //printf("P(%d): memory usage: %f GB\n", myid, (double)mem_usage/pow(1024.0,3));
    // size_t free, total;
    // cudaMemGetInfo(&free, &total);
    // printf("P(%d): free = %f, total = %f\n", myid, (double)free/pow(1024.0,3), (double)total/pow(1024.0,3));
  } // end GPU_SOLVE_PHASE
          ...
          ProblemData  *data_ptr )
{
   ...
   MPI_Bcast(&sdata_size, 1, MPI_INT, 0, HP_MPI_COMM);

   /* broadcast the data */
   sdata = hypre_TReAlloc(sdata, char, sdata_size);
   MPI_Bcast(sdata, sdata_size, MPI_CHAR, 0, HP_MPI_COMM);
      ...
      char *argv[] )
{
   ...
   int myid_default = myid;
   int stride = num_procs/8;
   int new_rank_id =  ((myid*stride)%num_procs)  + (myid*stride)/num_procs ;
   MPI_Comm_split(MPI_COMM_WORLD, 0, new_rank_id, &HP_MPI_COMM);

   char HNAME[256];
   int HNAME_SIZE = 256;
   MPI_Get_processor_name( &HNAME[0], &HNAME_SIZE );
   printf("old rank id= %d  new rank_id= %d, hostname %s\n",myid,new_rank_id, HNAME);



   MPI_Comm_rank(HP_MPI_COMM, &myid);

   // select one GPU per MPI rank
   int num_gpus;
   cudaGetDeviceCount(&num_gpus);
   if (num_gpus == 1) cudaSetDevice(0);
   else  cudaSetDevice(myid_default % num_gpus); 


   printf("old_rank_id = %d,  cpuID = %d, num_devices = %d\n",myid_default,sched_getcpu(),num_gpus);
   ...
   while ( (arg_index < argc))
   {
     ...
     else if ( strcmp(argv[arg_index], "-host") == 0 )
     {
       arg_index++;
       host_only = 1;
     } 
   ...
   if (build_matrix_type > 1 && build_matrix_type < 8)
   {
      ...
      hypre_PrintTiming("Setup matrix and rhs", &wall_time, HP_MPI_COMM);
   ...
   else 
   {
     ...
     while (arg_index < argc)
     {
       ...
       {
         arg_index++;
         rlx = atoi(argv[arg_index++]);
         if (rlx != 16 && rlx != 18) 
         {
           printf("rlx: only 16 and 18 are supported");
           return (1);
         }
       }
     ...
     HYPRE_SStructGridCreate(HP_MPI_COMM, data.ndim, data.nparts, &grid);
     for (part = 0; part < data.nparts; part++)
     {
       pdata = data.pdata[part];
       for (box = 0; box < pdata.nboxes; box++)
     ...
     HYPRE_SStructGraphCreate(HP_MPI_COMM, grid, &graph);
     ...
     HYPRE_SStructMatrixCreate(HP_MPI_COMM, graph, &A);
     ...
     HYPRE_SStructVectorCreate(HP_MPI_COMM, grid, &b);
     ...
     HYPRE_SStructVectorCreate(HP_MPI_COMM, grid, &x);
     ...
     hypre_EndTiming(time_index);
     hypre_PrintTiming("SStruct Interface", &wall_time, HP_MPI_COMM);
     hypre_FinalizeTiming(time_index);
     hypre_ClearTiming();
     fflush(NULL);
     ...
     hypre_TFree(values);
      ...
      if ((solver_id > -1) && (solver_id < 2))
      {
         ...
         if (solver_id == 0)
         {
            ...
            if (rlx == 16) // Cheby
            { 
              HYPRE_BoomerAMGSetRelaxType(par_precond, 16);
              HYPRE_BoomerAMGSetCycleRelaxType(par_precond, 16, 3);
              HYPRE_BoomerAMGSetCycleNumSweeps(par_precond, 1, 3);
              HYPRE_BoomerAMGSetRelaxOrder(par_precond, 0);
            }
            if (rlx == 18) // L1-Jacobi
            {
              HYPRE_BoomerAMGSetRelaxType(par_precond, 18);		// parallel Jacobi-L1 smoother
              HYPRE_BoomerAMGSetCycleRelaxType(par_precond, 18, 3);	// parallel Jacobi-L1 on coarse level as well
              HYPRE_BoomerAMGSetCycleNumSweeps(par_precond, 2, 1);	// 2 pre-sweeps
              HYPRE_BoomerAMGSetCycleNumSweeps(par_precond, 2, 2);	// 2 post-sweeps
              HYPRE_BoomerAMGSetCycleNumSweeps(par_precond, 2, 3);	// 2 coarsest-sweeps
              HYPRE_BoomerAMGSetRelaxOrder(par_precond, 0);
            }
         ...
         hypre_PrintTiming("Setup phase times", &wall_time, HP_MPI_COMM);
         hypre_FinalizeTiming(time_index);
         hypre_ClearTiming();
         fflush(NULL);
         ...
         size_t dev_free_begin, dev_free_end, dev_total;
         device_memInfo(&dev_free_begin, &dev_total);
         if (!host_only)
           GPU_SOLVE_PHASE = 1; // enable GPU vector operations

         // set the relevant device
         int device_count, my_device;
         if (GPU_SOLVE_PHASE) {
           device_count = device_numDevices();
           my_device = myid % device_count;
           device_setDevice(my_device);
         }
         device_report_memory();
         MPI_Barrier(HP_MPI_COMM);
         // upload hierarchy to device, note that we do not include that into solve phase timings
         upload_hierarchy(par_precond, rlx);
         upload_PCG(par_solver);
         MPI_Barrier(HP_MPI_COMM);
         double t2 = MPI_Wtime();
         if (myid == 0)
                 printf ("\n DATA ULOAD TIME= %e[s]\n\n", t2-t1);
         ...
         GPU_SOLVE_PHASE = 0; // disable GPU vector operations

         #if defined(LEVEL_TIMING)
           device_memInfo(&dev_free_end, &dev_total);
           int ranks_per_device = 1;
           if (myid == 0) {
             printf("CUDA memory usage per fine row: %.4e GB\n",((double)dev_free_begin-(double)dev_free_end)/1e9 / hypre_ParCSRMatrixNumRows((hypre_ParCSRMatrix *)par_A ) / ranks_per_device);
           }
           print_all_timing(hierarchy_num_levels(par_precond));
         #endif

         hypre_EndTiming(time_index);
#ifdef MPIPCONTROL
         MPI_Pcontrol(0,"summary_stop");
#endif

         hypre_PrintTiming("Solve phase times", &wall_time, HP_MPI_COMM);
         hypre_FinalizeTiming(time_index);
         hypre_ClearTiming();
         fflush(NULL);
      ...
      if ((solver_id > 1) && (solver_id < 4))
      {
         ...
         HYPRE_ParCSRGMRESCreate(HP_MPI_COMM, &par_solver);
                   ...
                   HYPRE_ParVector  *x_ptr)
{
   ...
   MPI_Comm_size(HP_MPI_COMM, &num_procs );
   MPI_Comm_rank(HP_MPI_COMM, &myid );
   ...
   A = (HYPRE_ParCSRMatrix) GenerateLaplacian(HP_MPI_COMM, 
                       ...
                       HYPRE_ParVector  *x_ptr)
{
   ...
   A = (HYPRE_ParCSRMatrix) GenerateLaplacian27pt(HP_MPI_COMM,
                               g_nx, g_ny, g_nz, P, Q, R, p, q, r, values, &rhs, &x);
                    ...
                    HYPRE_ParVector  *x_ptr)
{
   ...
   A = (HYPRE_ParCSRMatrix) GenerateVarDifConv(HP_MPI_COMM,
