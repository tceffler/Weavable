...
*/
   ...
   nvtxRangePushA("PCG Loop");
   while ((i+1) <= max_iter)
   {
     current_iteration = i;
     double start, end, ticks;
    ...
    precond(precond_data, A, r, s);
     ...
     (*(pcg_functions->Axpy))(1.0, s, p);
       ...
       int myid;
       MPI_Comm_rank(HP_COMM_WORLD, &myid);
       double time_taken = end-start, max_time;
       MPI_Reduce(&time_taken, &max_time, 1, MPI_DOUBLE, MPI_MAX, 0, HP_COMM_WORLD);
       if (myid == 0) {
         printf("ITERATION %d %lg\n", i, max_time);
       }
     #endif
   }
   nvtxRangePop();
