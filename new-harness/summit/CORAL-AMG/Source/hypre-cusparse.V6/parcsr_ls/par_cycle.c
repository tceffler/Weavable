                   ...
                   hypre_ParVector  **U_array   )
{
 ...
 /* Local variables  */ 
   ...
   int GPU_SOLVE_PHASE_REF = GPU_SOLVE_PHASE;
//#ifdef SWITCH_HOST_DEVICE
//   int SWITCH_LEVEL = 6;
//#endif
   ...
   double start, end; // , timing[20];
   int curr_level = level;
   device_synchronize();
   start = MPI_Wtime();
#endif
   static int first_call = 1;
   nvtxRangePushA("AMGCycle");
   char out_string[10];
   while (Not_Finished)
   {
     sprintf(out_string, "level%d", level);
     nvtxRangePushA(out_string);
#if defined(LEVEL_TIMING)
      // changed level -- stop timer, accumulate and restart
      if (level != curr_level) {
        device_synchronize();
        end = MPI_Wtime();
        if (current_iteration == 2)
          timing[curr_level] += end - start;
        curr_level = level;
        start = MPI_Wtime();
      }
#endif
      ...
      // if (my_id == 7) printf("Level = %d\n", level);
      // MPI_Barrier(comm);
      // hypre_CSRMatrix *A_diag = hypre_ParCSRMatrixDiag(A_array[level]);
      // if (my_id >= 112) 
      //   printf("L(%d),P(%d): NumRows(A_diag)=%d\n", level, my_id, hypre_CSRMatrixNumRows(A_diag));
      // MPI_Barrier(comm);
      // }
      if (num_levels > 1) 
      {
...
// #ifdef SWITCH_HOST_DEVICE
//          if ((fine_grid == (SWITCH_LEVEL) )  && (GPU_SOLVE_PHASE_REF == 1) ){
//       //      printf("level = %d, GPU_SOLVE_PHASE = %d  switch execution GPU->CPU\n",level,GPU_SOLVE_PHASE);
//             hypre_VectorSyncHost(U_array[fine_grid]->local_vector);
//             hypre_VectorSyncHost(F_array[fine_grid]->local_vector);
...
// #endif
      ...
      //      printf("level = %d, GPU_SOLVE_PHASE = %d  switch execution GPU->CPU\n",level,GPU_SOLVE_PHASE);
            ...
            hypre_VectorSyncHost(F_array[coarse_grid]->local_vector);
...
#ifdef SWITCH_HOST_DEVICE
         //LG copy U_array[coarse_grid] from CPU to GPU
         if ((fine_grid == SWITCH_LEVEL) && (GPU_SOLVE_PHASE_REF == 1) && (GPU_SOLVE_PHASE == 0)){
         //   printf("level = %d, GPU_SOLVE_PHASE = %d  switch execution CPU->GPU\n",level,GPU_SOLVE_PHASE);
            GPU_SOLVE_PHASE = 1;
...
#if defined(LEVEL_TIMING)
   // final case -- stop timer and accumulate
   device_synchronize();
   end = MPI_Wtime();
   if (current_iteration == 2) 
     timing[curr_level] += end-start;
