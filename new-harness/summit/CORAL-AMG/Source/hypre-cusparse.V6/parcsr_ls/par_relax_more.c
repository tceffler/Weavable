                            ...
                            hypre_ParVector *r    /*another temp vector */  )
{
   ...
   if (GPU_SOLVE_PHASE) {
     nvtxRangePushA("SyncDevice");
     if (!u_local->initialized) hypre_VectorSyncDevice(u_local);
     if (!f_local->initialized) hypre_VectorSyncDevice(f_local);
     if (!v_local->initialized) hypre_VectorSyncDevice(v_local);
     if (!r_local->initialized) hypre_VectorSyncDevice(r_local);
     nvtxRangePop();
   } 
   int GPU_SOLVE_PHASE_OLD = GPU_SOLVE_PHASE;
   // if (GPU_SOLVE_PHASE) {
   //   if (u_local->initialized) hypre_VectorSyncHost(u_local);
   //   if (f_local->initialized) hypre_VectorSyncHost(f_local);
   //   if (v_local->initialized) hypre_VectorSyncHost(v_local);
   //   if (r_local->initialized) hypre_VectorSyncHost(r_local);
   // } 
   // GPU_SOLVE_PHASE=0;
   ...
   if (!GPU_SOLVE_PHASE) {
     ...
     d_orig_u = device_get_vector(num_rows, 2);
   }
...
#pragma omp parallel for private(i) schedule(static) 
   ...
   else /* scaling! */
   {
      ...
      // hack to disable GPU memory allocation
      nvtxRangePushA("vectorCreate");
      GPU_SOLVE_PHASE = 0;
      ds = hypre_ParVectorCreate(hypre_ParCSRMatrixComm(A),
                                 hypre_ParCSRMatrixGlobalNumRows(A),
                                 hypre_ParCSRMatrixRowStarts(A));
      ...
      GPU_SOLVE_PHASE = GPU_SOLVE_PHASE_OLD;
      nvtxRangePop();

      nvtxRangePushA("device_get_vector");
      hypre_ParVectorSetPartitioningOwner(ds,0);
      ds_data = hypre_VectorData(hypre_ParVectorLocalVector(ds));
      hypre_Vector *ds_local = hypre_ParVectorLocalVector(ds);
      ds_local->d_data = device_get_vector(ds_local->size, 0);
      ds_local->initialized = 1;

      hypre_ParVectorSetPartitioningOwner(tmp_vec,0);
      tmp_data = hypre_VectorData(hypre_ParVectorLocalVector(tmp_vec));
      hypre_Vector *tmp_local = hypre_ParVectorLocalVector(tmp_vec);
      tmp_local->d_data = device_get_vector(tmp_local->size, 1);
      tmp_local->initialized = 1;
      nvtxRangePop();
    ...
    /* get ds_data and get scaled residual: r = D^(-1/2)f -
      ...
      if (GPU_SOLVE_PHASE) {
        device_cheby_loop1(A_diag, ds_local->d_data, r_local->d_data, f_local->d_data);
      } else {
#ifdef HYPRE_USING_OPENMP
#pragma omp parallel for private(j,diag) schedule(static) 
      ...
      if (GPU_SOLVE_PHASE) {
        device_cheby_loop2(r_local->d_data, ds_local->d_data,
                           tmp_local->d_data, d_orig_u,
                           u_local->d_data, coefs[cheby_order], num_rows);
      } else {
#ifdef HYPRE_USING_OPENMP
#pragma omp parallel for private(j) schedule(static) 
#endif
...
#pragma omp parallel for private(j) schedule(static) 
      ...
      for (i = cheby_order - 1; i >= 0; i-- ) 
      {
         /* v = D^(-1/2)AD^(-1/2)u */
        if (GPU_SOLVE_PHASE) {
          device_cheby_loop3(tmp_local->d_data, ds_local->d_data, 
                             u_local->d_data, num_rows);
        } else {
#ifdef HYPRE_USING_OPENMP
#pragma omp parallel for private(j) schedule(static) 
#endif
         ...
         if (GPU_SOLVE_PHASE) {
           device_cheby_loop4(ds_local->d_data, v_local->d_data, 
                              u_local->d_data, r_local->d_data,
                              mult, num_rows);
         } else {
#ifdef HYPRE_USING_OPENMP
#pragma omp parallel for private(j,tmp_d) schedule(static) 
      ...
      if (GPU_SOLVE_PHASE) {
        device_cheby_loop5(u_local->d_data, d_orig_u,
                           ds_local->d_data, num_rows);
      } else {
#ifdef HYPRE_USING_OPENMP
#pragma omp parallel for private(j) schedule(static) 
#endif
      ...
      // hack for disable pinned register
      GPU_SOLVE_PHASE = 0;
      hypre_ParVectorDestroy(ds);
   ...
   // if (GPU_SOLVE_PHASE_OLD) {
   //   hypre_VectorSyncDevice(u_local);
   // }
...

{
    ...
    double *d_temp, *d_Vtemp_data;
    if (GPU_SOLVE_PHASE) {
      // check if vector needs to be copied to GPU (only once)
      if (!u_local->initialized) hypre_VectorSyncDevice(u_local);
      if (!Vtemp_local->initialized) hypre_VectorSyncDevice(Vtemp_local);

      d_temp = device_get_vector(n, 0);
      d_Vtemp_data = Vtemp_local->d_data;
    }
    ...
    *-----------------------------------------------------------------*/
    if (!GPU_SOLVE_PHASE) {
      ...
      hypre_SeqVectorCopy(u_local, Vtemp_local); 
    }
    ...
    if (num_procs > 1)
    {
     ...
     // setup comm buffers, copy temp vector and launch diag spmv
     if (GPU_SOLVE_PHASE) {
       int send_size = hypre_ParCSRCommPkgSendMapStart(comm_pkg, num_sends);
       int *send_maps = NULL;

       if (!device_has_send_maps(A_diag, 0)) {
       ...
       // if (first_call && my_id >= 112) {
       // char id_msg[100];
       // sprintf(id_msg, "P(%d): ", my_id);
       // for (int i = 0; i < num_sends; i++) {
       //   char msg[20];
       //   sprintf(msg, "%d, ",  hypre_ParCSRCommPkgSendProc(comm_pkg, i));
       //   strcat(id_msg, msg);
       // }
       // printf("%s\n", id_msg);
       // first_call = 0;
       ...
       device_set_stream(1);
       device_create_comm_buffer(A_diag, send_size, send_maps, v_buf_data, u_local->d_data);

       // copy current approx into temp vector
       device_set_stream(2);
       device_memcpyAsync(Vtemp_local->d_data, u_local->d_data, sizeof(double) * u_local->size, D2D);

       // launch diag spmv to occupy gpu
       device_spmv(A_diag, n, hypre_CSRMatrixNumCols(A_diag), hypre_CSRMatrixNumNonzeros(A_diag), -relax_weight, A_diag_i, A_diag_j, A_diag_data, d_Vtemp_data, n, 0.0, d_temp, n, 0);

       // make sure comm buffers are on gpu
       device_set_stream(1);
       device_sync_stream();
    ...
    if (relax_points == 0 || cf_marker == NULL)
    {
       if (GPU_SOLVE_PHASE) {
         // copy external data
         double *d_Vext_data = device_get_vector(num_cols_offd, 0);
         device_set_stream(1);
         device_memcpyAsync(d_Vext_data, Vext_data, sizeof(double) * num_cols_offd, H2D); 
         device_sync_stream();

         // launch offd spmv 
         device_set_stream(2);
         device_spmv(A_offd, n, hypre_CSRMatrixNumCols(A_offd), hypre_CSRMatrixNumNonzeros(A_offd), -relax_weight, A_offd_i, A_offd_j, A_offd_data, d_Vext_data, num_cols_offd, 1.0, d_temp, n, 0);

         // combine offd/diag
         double *d_u_data = u_local->d_data;
         double *d_f_data = f_local->d_data;
         device_l1_norm(A_diag, n, hypre_CSRMatrixNumCols(A_diag), d_u_data, d_f_data, l1_norms, d_temp, relax_weight);
