...
int main(int argc, char **argv) {
  /* local variables */
  char fname_times[1024], buf[32], outname[255], hostname[64];
  int i,j, pid;
  ...
  sprintf(outname,"fwq_times.%s.%d", hostname, pid);
   ...
   while (1) {
     ...
     case 't':
       ...
       fprintf(stderr,"ERROR: fwq not compiled with pthreads support.\n");
       exit(EXIT_FAILURE);
#endif
  ...
  samples = calloc((size_t)(numsamples*numthreads), sizeof(unsigned long long));
  assert(samples != NULL);
  ...
  if (use_threads == 1) {
    ...
    assert(threads != NULL);
    thread_args = calloc((size_t)numthreads, sizeof(fwq_args));
    ...
    for (i=0;i<numthreads;i++) {
      ...
      if (i) {
        ...
        rc = pthread_create(&threads[i], NULL, fwq_core, (void *)thr_arg);
        if (rc) {
  ...
  if (use_stdout == 1) {
    ...
    for (j=0;j<numthreads;j++) {
      sprintf(fname_times,"%s_%d.dat",outname,j);
