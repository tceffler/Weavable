...
//Work around compilers should be able to handle this better in the future.  If not this is still a good change for CPU version
#if 1
#if 1
#define FMINF(a,b) (((a)<(b))?(a):(b))
#define FMAXF(a,b) (((a)>(b))?(a):(b))
#else
#define FMINF(a,b) fminf(a,b)
...
void cm( int count, float *xx, float *yy, float *zz, float *mass, float *xmin, float *xmax, float *xc)
{
  ...
  double w, x = 0, y = 0, z = 0, m = 0;
  float xmin0, xmin1, xmin2;
  float xmax0, xmax1, xmax2;
  xmin0 = xmax0 = xx[0];
  xmin1 = xmax1 = yy[0];
  xmin2 = xmax2 = zz[0];
  w = mass[0];
  ...
  for (int i = 0; i < count; ++i) {
      xmin0 = FMINF(xmin0, xx[i]);
      xmax0 = FMAXF(xmax0, xx[i]);
      xmin1 = FMINF(xmin1, yy[i]);
      xmax1 = FMAXF(xmax1, yy[i]);
      xmin2 = FMINF(xmin2, zz[i]);
      xmax2 = FMAXF(xmax2, zz[i]);
