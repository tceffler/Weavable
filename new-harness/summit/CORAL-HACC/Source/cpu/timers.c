...
/* need locks if using inside threaded regions              */
/*----------------------------------------------------------*/
/* Fortran :                                                */
...
static char code_block_label[MAX_CODE_BLOCKS][MAX_LABEL_LEN];
...
void timer_beg(char * f_label, int length)
{
   ...
   if (!initialized)
   {
       ...
       memset(code_block_label, '\0', MAX_CODE_BLOCKS*MAX_LABEL_LEN);
