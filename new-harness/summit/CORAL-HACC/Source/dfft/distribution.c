...
// This actually does the work.
///
static void redistribute_2_and_3(const complex_t * restrict a,
                                 complex_t * restrict b,
                                 distribution_t * restrict d,
                                 int direction,
				 int z_dim)
{
  ...
  for (int p = 0; p < npeers; ++p) {
    ...
    if(direction == REDISTRIBUTE_3_TO_2){
      ...
      int64_t dim1 = d->process_topology_3.n[1];
      int64_t dim2 = d->process_topology_3.n[2];
	...
	case 0:
	  for(int i1=d3_array_start[x_dim];i1<d3_array_start[x_dim]+subsizes[x_dim];i1++){//perhaps x_dim
       //   for(int i2=d3_array_start[y_dim];i2>d3_array_start[y_dim]-subsizes[y_dim];i2--){//perhaps y_dim
	    for(int i2=d3_array_start[y_dim]-subsizes[y_dim]+1; i2<=d3_array_start[y_dim];i2++){//perhaps y_dim
              int64_t sndx2 = (d3_array_start[y_dim] - i2)*subsizes[x_dim]*subsizes[z_dim];
              int64_t sndx1 = sndx2 + (i1 - d3_array_start[x_dim])*subsizes[z_dim] - d3_array_start[z_dim];
	      for(int i0=d3_array_start[z_dim];i0<d3_array_start[z_dim]+subsizes[z_dim];i0++){//perhaps z_dim
		int64_t local_indx = dim2*(dim1*i0 + i1) + i2;
	//	assert(local_indx < dims_size);
	//	assert(ch_indx <chunk_size && ch_indx >= 0 && local_indx>=0 && local_indx < dims_size);
		d->d3_chunk[sndx1 + i0] = a[local_indx];
	      }
	    }
	  }
	  break;
    ...
    if(direction == REDISTRIBUTE_3_TO_2){
      ...
      int64_t ch_indx=0;
      int dims_size=pencil_dims[0]*pencil_dims[1]*pencil_dims[2];
      int64_t dim1 = pencil_dims[1];
      int64_t dim2 = pencil_dims[2];
	...
	//for(int i2=d3_array_start[y_dim];i2>d3_array_start[y_dim]-subsizes[y_dim];i2--){
	  for(int i2=d3_array_start[y_dim]-subsizes[y_dim]+1; i2<=d3_array_start[y_dim]; i2++){
            int64_t sndx2 = (d3_array_start[y_dim] - i2)*subsizes[x_dim]*subsizes[z_dim];
            int64_t sndx1 = sndx2 + (i1 - d3_array_start[x_dim])*subsizes[z_dim] - d3_array_start[z_dim];
	  ...
	  //  assert(local_indx < dims_size);
	  //  assert(ch_indx <chunk_size && ch_indx >= 0 && local_indx>=0 && local_indx < dims_size);
	      b[local_indx] = d->d3_chunk[sndx1 + i0];
	    }
	  }
	}
      }
